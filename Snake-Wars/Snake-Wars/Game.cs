...
namespace Snake_Wars
{
    class Game
    {
        ...
        public void InitData(int speed)
        {
            ...
            data = new Blocks[XRows, YRows];    //方块的位置
        ...
        void Move()//移动
        {
            switch (dir)
            {
                case Directions.Up:
                    ...
                    {
                        Blocks front = data[snake.Head1.X - 1, snake.Head1.Y];
                ...
                case Directions.Left:
                    if (snake.Head1.Y > 0)
                    {
                        Blocks front = data[snake.Head1.X, snake.Head1.Y - 1];
                        if (checkFood(front))           //是否是食物
                        {
                            snake.AddSnake(new Snake(front, snake.Color), data);
                            Score += 20;
        ...
        bool checkFood(Blocks obj)//判断是否是食物
        {
            if (obj is Food)
                return true;
            else
        ...
        bool checkBody(Blocks obj)//判断是否是蛇
        {
            if (obj is Snake)
                return true;
            else
